...
The inbox currently displays all accepted notifications in chronological order. **No mailbox-like status functionality** (read/unread, archived, deleted, flagged) is implemented. This is intentional - the system focuses on LDN compliance rather than email-client features. For production use cases requiring status tracking, consider implementing:
- Status flags in `i_notifications` table (`is_read`, `is_archived`, `is_deleted`)
- UI filters for status-based views
- User-specific notification states (multi-user read tracking)

### Form Builder (Experimental)

The form builder is an **experimental feature** demonstrating a practical approach to JSON-LD payload creation in the context of LDN inboxes and ActivityStreams. While far from perfect, it showcases:
- Context-aware human-agent interaction with semantic notifications
- Auto-fill UX reducing duplicate data entry
- Live preview for immediate feedback
- Educational comments bridging PHP and JavaScript concepts

This approach may be viable for **non-technical users** who need to send Event Notifications without understanding JSON-LD internals.

### Notification and Resource IRIs

**Resource IRIs limitation:**

Resource IRIs (e.g., `http://localhost:8081/resources/1`) are still **semantic identifiers only** - they are **not dereferenceable endpoints**. Clicking these IRIs will return 404. They serve as unique identifiers in JSON-LD notifications, not as actual web resources. To link to real hosted content, use the "Content URL" field.

**Potential enhancements:**
- **Resource endpoint** to make resource IRIs dereferenceable (similar to notifications)
- **File upload library** integration (store actual files, auto-generate IRIs)
- **Metadata extraction** from uploaded files (PDF title, image dimensions, etc.)
- **Resource versioning** to track updates
- **Direct resource-to-inbox linking** during upload
- **Preview generation** for uploaded content

### Queue Implementation for Production

For **robust production deployment**, implement an asynchronous message queue for outgoing notifications:

**Recommended:**
- **RabbitMQ** or **Kafka** for reliable message delivery
- Retry logic with exponential backoff
- Dead letter queues for failed deliveries
- Batch processing for high-volume scenarios
- Separate worker processes for delivery attempts

Current implementation uses synchronous cURL which blocks PHP execution and doesn't handle failures gracefully.
...
- **Laravel** - Developer-friendly, built-in queue support
- **Slim Framework** - Lightweight, good for API-focused applications
...
Currently, notifications are displayed as **plain JSON** with basic field extraction (type, actor, object, target). **Visualization improvements needed:**

**Request-Response Sequence Visualization:**
- Thread view showing notification chains (via `inReplyTo`, `correlationId`)
- Timeline/conversation visualization (like email threads)
- Graph view showing actor → target relationships
- Color-coded activity types (Create=green, Delete=red, etc.)
- Expandable/collapsible JSON with syntax highlighting

**Key Component Extraction:**
- Visual badges for extracted fields (actor avatar, object type icon)
- Inline preview of referenced resources
- Interactive graph of notification flows
- Status indicators for delivery state

**Potential Libraries:**
- D3.js for graph visualization
- Mermaid.js for sequence diagrams
- JSON Viewer libraries for interactive JSON display
- Timeline.js for chronological views
...
**PostgreSQL 9.4+** offers comparable JSON capabilities and may be preferred for:

**Migration Path:**
- Replace MySQL-specific syntax with PostgreSQL equivalents
- Change JSON operators: `->>'$.field'` → `->>'field'` or `->> 'field'`
- Update ENUM types to CHECK constraints or native PostgreSQL ENUMs
- Modify AUTO_INCREMENT to SERIAL or IDENTITY columns
- Replace `INET_ATON()`/`INET_NTOA()` with native INET/CIDR types

**PostgreSQL Advantages:**
- **Better JSON performance** at scale (JSONB indexing with GIN)
- **Advanced JSON operators** (containment @>, path queries #>, etc.)
- **Full-text search** integration with JSON fields
- **Stricter SQL compliance** and data integrity
...
- Replace relational joins with embedded documents or references
- Use MongoDB aggregation pipeline for complex queries
- Implement ACL validation in application layer or with MongoDB roles

**MongoDB Advantages:**
- **Native JSON/BSON storage** - zero impedance mismatch
- **Flexible schema evolution** - add fields without migrations
- **Horizontal scalability** via sharding (distribute by inbox_id)
- **Aggregation pipeline** for complex ActivityStreams queries
- **Built-in replication** (replica sets) for high availability
- **Time-series collections** for notification archival
- **GridFS** for storing large JSON-LD payloads or attachments

**MongoDB Disadvantages:**
- Loss of ACID transactions across collections (before 4.0)
- No foreign key constraints (enforce in application layer)
- More complex ACL implementation (no JOIN-based validation)
- Larger storage footprint (BSON overhead)
- Different query paradigm (learning curve)
...
# Notifications Collection
{
  _id: ObjectId("..."),
  inbox_id: ObjectId("..."),
  notification_iri: "http://localhost:8081/notifications/n1",
  sender: {
    actor_iri: "https://example.org/user/1",
    verified: true,
  ...
  body_jsonld: {
    "@context": "https://www.w3.org/ns/activitystreams",
    "type": "Offer",
    "actor": { "id": "...", "inbox": "...", "name": "...", "type": "Person" },
    "object": "..."
  },
  as_type: "Offer",
  as_object_iri: "http://resource.org/123",
  digest_sha256: BinData(0, "..."),
  status: "accepted",
  received_at: ISODate("2025-10-05T12:00:00Z"),
  http_meta: {
    origin_ip: "192.168.1.100",
    user_agent: "curl/7.68.0",
    headers: { "Host": "localhost:8081", "Signature": "..." }
  }
}

# ACL Rules Embedded or Referenced
{
  _id: ObjectId("..."),
  inbox_id: ObjectId("..."),
  acl_rules: [
    { rule_type: "allow", match_kind: "domain_suffix", match_value: "orcid.org" },
    { rule_type: "deny", match_kind: "exact_actor", match_value: "https://spam.example/bot" }
  ]
}
```
...
```javascript
# Find all Offer notifications
db.notifications.find({ "body_jsonld.type": "Offer" })

# Find notifications from specific domain
db.notifications.find({ "sender.actor_iri": /orcid\.org$/ })

# Aggregation pipeline for correlation tracking
db.notifications.aggregate([
  { $match: { "body_jsonld.correlationId": "req-abc-123" } },
  { $lookup: { from: "inboxes", localField: "inbox_id", foreignField: "_id", as: "inbox" } },
  { $sort: { received_at: 1 } }
])

# ACL validation in application
const rules = await db.acls.findOne({ inbox_id: inboxObjectId });
// JavaScript validation logic (similar to current acl_allows() function)
```


**Hybrid Approach:**
- Keep **MySQL/PostgreSQL** for user accounts, inboxes, ACLs (relational integrity)
- Use **MongoDB** for notifications only (JSON-LD storage and queries)
